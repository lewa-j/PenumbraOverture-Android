...
cNotebookState_Front::cNotebookState_Front(cInit *apInit, cNotebook *apNotebook) : iNotebookState(apInit, apNotebook)
{
	...
	mvOptions[0].mvPos = cVector3f(screenSize.x / 2, screenSize.y / 2 - 80, 70);

	mvOptions[1].msText = kTranslate("Notebook", "PickedNotes");
	mvOptions[1].mvPos = cVector3f(screenSize.x / 2, screenSize.y / 2 - 20, 70);
...
cNotebookState_TaskList::cNotebookState_TaskList(cInit *apInit, cNotebook *apNotebook) : iNotebookState(apInit, apNotebook)
{
	...
	cVector3f vBookPos(screenSize.x / 2 - vBookSize.x / 2, screenSize.y / 2 - vBookSize.y / 2, 60);
...
{
	const cVector2f screenSize = mpInit->mpGame->GetGraphics()->GetLowLevel()->GetVirtualSize();
	cVector3f vPos(254, screenSize.y / 2 - 190, 70);
	...
	while (it.HasNext())
	{
		...
		mpTextFont->Draw(vPos + cVector3f(-12, 0, 0), 14, cColor(0.0f, 0.0f, 0.0f, 0.9f), eFontAlign_Left, _W("-"));
	...
	//////////////////////////////////
	// Page number
	mpTextFont->Draw(cVector3f(screenSize.x / 2, screenSize.y / 2 + 190, 16), 15, cColor(0.05f, mpNotebook->GetAlpha()), eFontAlign_Center,
	...
	for (size_t i = 0; i < mvOptions.size(); ++i)
	{
		...
		mpDrawer->DrawGfxObject(mpOptionsImage[i], mvOptions[i].mvPos,
			cVector2f(mvOptions[i].mRect.w, mvOptions[i].mRect.h), cColor(1, mpNotebook->GetAlpha()));
	}
}
...
void cNotebookState_NoteList::OnEnterState(int alLastState)
{
	...
	const cVector2f screenSize = mpInit->mpGame->GetGraphics()->GetLowLevel()->GetVirtualSize();
	int lCount = 0;
	cVector3f vPos(242, screenSize.y / 2 - 189, 70);
	...
	while (it.HasNext())
	{
		...
		{
			lCount = 0;
			vPos = cVector3f(242, screenSize.y / 2 - 189, 70);
		}
...
void cNotebookState_NoteList::OnDraw()
{
	...
	while (it.HasNext())
	{
		...
		cVector3f vTabPos(screenSize.x / 2 - 175, pNote->mRect.y, 68);
	...
	//////////////////////////////////
	// Page number
	mpTextFont->Draw(cVector3f(screenSize.x / 2, screenSize.y / 2 + 190, 16), 17, cColor(0.05f, mpNotebook->GetAlpha()), eFontAlign_Center,
...
void cNotebookState_Note::OnDraw()
{
	...
	//////////////////////////////////
	// Draw page text
	cVector3f vPos(235, screenSize.y / 2 - 193, 70);
	for (int i = mvPages[mlCurrentPage].mlStart; i < mvPages[mlCurrentPage].mlEnd; ++i)
	{
		mpTextFont->Draw(vPos, mvSize, cColor(0, 1.0f * mpNotebook->GetAlpha()), eFontAlign_Left, mvRows[i].c_str());
...
void cNotebook::OnDraw()
{
	...
	//Book types
	cVector2f vBookSize(350, 460);
	cVector3f vBookPos(screenSize.x / 2 - vBookSize.x / 2, screenSize.y / 2 - vBookSize.y / 2, 10);
...
void cNotebook::OnMouseDown(eMButton aButton)
{
	...
	bookRect.w = 350;
	bookRect.h = 460;
	bookRect.x = screenSize.x / 2 - bookRect.w / 2;
	bookRect.y = screenSize.y / 2 - bookRect.h / 2;
